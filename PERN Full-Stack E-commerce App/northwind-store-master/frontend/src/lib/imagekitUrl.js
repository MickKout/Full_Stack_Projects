 ...
 */
function buildHarborTextLayer({ w, h }) {
  ...
  return `l-text,i-Harbor%20Market,fs-${fs},co-FFFFFF,bg-0F172A90,pa-8_12,lx-N14,ly-14,lap-top_right,l-end`;
}
...
function buildTrSegment({ w, h, q = 80, f = "auto", crop, watermark = false }) {
  ...
  return `${base}:${buildHarborTextLayer({ w, h })}`;
}
...
/**
 * Same optimizations as {@link imageKitOptimizedUrl} plus Harbor Market text overlay (for share/download).
